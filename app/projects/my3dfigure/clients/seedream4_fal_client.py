...
class Seedream4FalClient:
    ...
    async def _upload_local_image(self, image_path):
        ...
        try:
            ...
            if hasattr(self.client, 'upload_file'):
                url = self.client.upload_file(image_path)
            elif hasattr(self.client, 'upload'):
                url = self.client.upload(image_path)
            else:
    ...
    async def generate_image(self, prompt, input_image_paths=None, seed=None, art_style=None, aspect_ratio="1:1", **kwargs):
        ...
        print("提交任务到Fal.ai...")
        # 新版 self.client.submit(id, arguments={...}) 或直接传参
        # 根据官方文档通常是 self.client.submit("model/id", arguments={...})
        # 但报错说 unexpected keyword argument 'arguments'，
        # 那么可能是直接 self.client.submit("model/id", prompt="...", ...)
        # 或者 self.client.submit("model/id", {...}) 作为位置参数

...
async def upscale_image_with_replicate(
    ...
    try:
        with ExitStack() as stack:
            ...
            # 使用replicate.Client(api_token=APIConfig.REPLICATE_API_TOKEN)获取完整的预测信息
            try:
                # 创建replicate客户端
                client = replicate.Client(api_token=APIConfig.REPLICATE_API_TOKEN)
                ...
                output_url = await asyncio.to_thread(
                    replicate.Client(api_token=APIConfig.REPLICATE_API_TOKEN).run,
...
def generate_upscale_filename(original_filename: str, scale: int) -> str:
    """生成放大后的文件名"""
    # 从配置中获取算法值
    from app.projects.my3dfigure.core.config import AlgorithmConfig

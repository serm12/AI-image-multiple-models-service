...
def _task_row(task: dict) -> str:
    ...
    source_page_url = _text(task.get("source_page_url"))
    if source_page_url:
        parsed_source = urlparse(str(task.get("source_page_url")))
        source_label = f"{parsed_source.netloc}{parsed_source.path or '/'}"
        source_link = (
            f'<a href="{source_page_url}" title="{source_page_url}" target="_blank" '
            f'rel="noopener noreferrer">{_text(source_label)}</a>'
        )
    else:
...
table{{width:100%;border-collapse:collapse;min-width:1715px;table-layout:fixed}}th,td{{padding:13px 12px;border-bottom:1px solid #edf0f5;text-align:left;vertical-align:middle}}th{{position:sticky;top:0;z-index:2;background:#f8fafc;font-size:12px;color:#64748b;white-space:nowrap}}tbody tr{{height:94px}}tbody tr:hover{{background:#fafcff}}
th:nth-child(1){{width:180px}}th:nth-child(2){{width:105px}}th:nth-child(3),th:nth-child(4){{width:165px}}th:nth-child(5){{width:90px}}th:nth-child(6){{width:170px}}th:nth-child(7){{width:125px}}th:nth-child(8){{width:90px}}th:nth-child(9){{width:170px}}th:nth-child(10){{width:145px}}th:nth-child(11){{width:115px}}th:nth-child(12){{width:265px}}

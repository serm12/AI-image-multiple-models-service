...
async def generate_image_async(
    ...
    traffic_source: str | None = Form(None),
    traffic_referrer: str | None = Form(None),
    traffic_landing_path: str | None = Form(None),
    utm_source: str | None = Form(None),
    utm_medium: str | None = Form(None),
    utm_campaign: str | None = Form(None),
    utm_term: str | None = Form(None),
    ...
    try:
        ...
        params = {
            ...
            "source_page_title": str(source_page_title or "").strip()[:500],
            "source_page_type": str(source_page_type or "").strip()[:50],
            ...
            "traffic_source": str(traffic_source or "").strip()[:120],
            "traffic_referrer": str(traffic_referrer or "").strip()[:500],
            "traffic_landing_path": str(traffic_landing_path or "").strip()[:500],
            "utm_source": str(utm_source or "").strip()[:120],
            "utm_medium": str(utm_medium or "").strip()[:120],
            "utm_campaign": str(utm_campaign or "").strip()[:255],
            "utm_term": str(utm_term or "").strip()[:255],
            "utm_content": str(utm_content or "").strip()[:255],
